...
\newenvironment{didas}{% 				% Didascálias antes de \part
			\pagebreak%
			\blankAtodd\mbox{}%
			%\baselineskip=.7\baselineskip%
			\vfill%
			\thispagestyle{empty}%
			\footnotesize\parindent0pt\paragraph{Introdução}}{\par} 
\newenvironment{resumo}{% 				% Resumos de abertura de capítulo
			\footnotesize
			%\baselineskip=.7\baselineskip
			\raggedright
			\parindent0pt\paragraph{Nota do organizador}}{\par\medskip} 

% Tirar indentação depois do resumo
\makeatletter
\newcommand*\NoIndentAfterEnv[1]{%
  \AfterEndEnvironment{#1}{\par\@afterindentfalse\@afterheading}}
\makeatother

\NoIndentAfterEnv{resumo}

% Comando para fazer subtítulo
\newcommand{\subtitulo}[1]{\NoCaseChange{\textnormal{\break\formular\normalsize\itshape#1}}}
\newcommand{\chapterspecial}[2]{%
  \chapter[#1]%
